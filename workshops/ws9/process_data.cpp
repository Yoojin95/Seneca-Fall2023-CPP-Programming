...
namespace sdds
{
	...
	ProcessData::ProcessData(const std::string& filename, int n_threads) {
		...
		{
			unsigned counter = 0;
			input.read(reinterpret_cast<char*>(&total_items), sizeof(int));
			data = new int[total_items];
			while (input)
			{
				input.read(reinterpret_cast<char*>(&data[counter++]), sizeof(int));
			}
		}
		input.close();
	...
	{
		auto p = std::bind(computeAvgFactor, placeholders::_1, placeholders::_2, total_items, placeholders::_3);
		std::vector<std::thread> threads;

		for (auto i = 0; i < num_threads; i++)
		{
			threads.push_back(std::thread(p, &data[p_indices[i]], (p_indices[i + 1] - p_indices[i]), std::ref(averages[i])));
		}
		...
		auto p2 = std::bind(computeVarFactor, placeholders::_1, placeholders::_2, total_items, avg, placeholders::_3);
		std::vector<std::thread> threads2;
		for (auto i = 0; i < num_threads; i++)
		{
			threads2.push_back(thread(p2, &data[p_indices[i]], (p_indices[i + 1] - p_indices[i]), std::ref(variances[i])));
		}
		...
		ofstream outFile(target_file.c_str(), ios::out | ios::binary);
		if (outFile)
		{
			outFile.write((const char*)(&total_items), sizeof(int));
			for (auto i = 0; i < total_items; i++)
			{
				outFile.write((const char*)(&data[i]), sizeof(int));
			}
